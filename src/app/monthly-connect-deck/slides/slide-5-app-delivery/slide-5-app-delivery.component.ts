...
@Component({
    selector: 'app-slide-5-app-delivery',
    standalone: true,
    imports: [CommonModule],
    templateUrl: './slide-5-app-delivery.component.html',
    styleUrls: ['./slide-5-app-delivery.component.css']
})
export class Slide5AppDeliveryComponent implements OnInit, OnChanges {
    @Input() generalInfo: GeneralInfo | null = null;
    @Input() deliveryMetrics: DeliveryMetric[] = [];

    appMetrics: DisplayMetric[] = [];
    ...
    private processData() {
        ...
        const rawMetrics = this.deliveryMetrics.filter(m => m.stream === 'App Platform');
        ...
        this.appMetrics = rawMetrics.map((m, index) => {
            ...
            return {
                ...
                spilled,
                statusClass,
                displayStatus: displayStatus === 'Deployed' ? 'Success' : (displayStatus || 'Planned'),
                iconHtml,
                sprintDuration,
                deliveryPct: pct
            };
